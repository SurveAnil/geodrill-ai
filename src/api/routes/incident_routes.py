...
async def get_well_details(well_id: str) -> Dict[str, Any]:
    ...
    """
    well = await run_in_threadpool(db_service.get_well, well_id)
    if not well:
        raise HTTPException(status_code=404, detail=f"Well '{well_id}' not found.")
    events = await run_in_threadpool(db_service.get_well_events, well_id)
    return {
        "well": well,
        "events": events,
    }
...
async def correlate_near_incidents(
    ...
    depth_m: float = Query(..., ge=0.0, le=100000.0, description="Planned measured depth in metres"),
    window_m: float = Query(100.0, gt=0.0, le=10000.0, description="Depth window radius (+/- metres)"),
    formation: Optional[str] = Query(None, max_length=200, description="Optional target formation filter"),
    radius_km: Optional[float] = Query(
        None, gt=0.0, le=500.0,
        description="Optional physical offset radius; requires coordinates for the target well",
    ),
) -> List[Dict[str, Any]]:
    """
    ...
    return await run_in_threadpool(
        db_service.query_events_near,
        well_id=well_id, depth_m=depth_m, window_m=window_m, formation=formation,
        radius_km=radius_km,
    )
...
async def risk_check(
    ...
    current_depth_m: float = Query(..., ge=0.0, le=100000.0, description="Current measured depth in metres"),
    formation: Optional[str] = Query(None, max_length=200, description="Current geological formation (optional)"),
    lookahead_m: float = Query(50.0, gt=0.0, le=10000.0, description="How far ahead to scan for historical incidents (metres)"),
) -> Dict[str, Any]:
    """
    ...
    ahead_events = await run_in_threadpool(
        correlate_ahead, active_well_id=well_id, current_depth_m=current_depth_m,
        formation=formation, lookahead_m=lookahead_m,
    )

    # Score the risk
    risk_result = await run_in_threadpool(score_risk, ahead_events)
