  ...
  return (
    <div className="min-h-screen bg-[#090D16] text-slate-100">
      ...
      <div className="flex">
        ...
        <aside aria-label="Primary navigation" className={`${mobileOpen ? 'translate-x-0' : '-translate-x-full'} fixed inset-y-0 left-0 z-[60] w-72 border-r border-slate-800 bg-[#0B1120] pt-3 shadow-2xl transition-transform md:static md:translate-x-0 md:pt-0 ${collapsed ? 'md:w-16' : 'md:w-56'} shrink-0 min-h-[calc(100vh-64px)]`}>
          ...
          <div className="p-3 space-y-1">
            {navigation.map(({ href, label, icon: Icon }) => {
              ...
              return <Link key={href} href={href} onClick={() => setMobileOpen(false)} aria-label={`Navigate to ${label}`} title={collapsed ? label : undefined} className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition ${active ? 'bg-cyan-500/15 text-cyan-300 border border-cyan-500/25' : 'text-slate-400 hover:bg-slate-800/70 hover:text-white'}`}>
