...
export function AlertDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <div className="fixed inset-y-0 right-0 z-[60] w-full max-w-md border-l border-slate-700 bg-[#0B1120] p-5 shadow-2xl">
    ...
    <p className="mt-6 rounded-lg border border-slate-800 bg-slate-900/60 p-4 text-sm text-slate-400">No active backend alerts. Simulated risk state remains visible on the Overview and Telemetry pages.</p>
  </div>;
}
