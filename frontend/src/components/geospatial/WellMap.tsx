...
export const WellMap: React.FC<WellMapProps> = ({
  ...
  return (
    <div className="w-full h-full min-h-[260px] relative rounded-lg overflow-hidden border border-slate-800 bg-[#090D16]">
      ...
      {!cartoTileUrl && (
        <div className="absolute top-2 left-2 right-2 z-[400] rounded border border-amber-700/70 bg-[#0B1120]/95 px-3 py-2 text-[11px] text-amber-200 shadow-md">
          Demo map state: configure <code>NEXT_PUBLIC_CARTO_API_KEY</code> to load the CARTO Voyager basemap.
        </div>
      )}
